#!/usr/bin/env python3
"""
Replace the broken HomePage.tsx with a clean version that includes socket debugging tools
"""

import os

def replace_clean_homepage():
    """Replace HomePage.tsx with a clean, working version"""
    ...
    print("🔧 Replacing HomePage.tsx with clean version...")
    
    # The clean HomePage content from the artifact
    clean_content = '''import React, { useState, useEffect } from 'react';
...
export default HomePage;'''
    ...
    backup_path = home_page_path + '.broken'
    try:
    ...
    print("\n✅ Successfully replaced HomePage.tsx with clean version!")
    print("\n🔧 Features included:")
    print("  1. Socket Debug Panel at the top - shows connection status, count, and history")
    print("  2. Manual connect/disconnect buttons for testing")
    print("  3. Console logging buttons for detailed socket inspection")
    print("  4. Real-time connection monitoring (updates every second)")
    print("  5. Connection history tracking")
    print("  6. Proper JSX structure (no syntax errors)")
    
    return True

if __name__ == "__main__":
    success = replace_clean_homepage()
    if success:
        print("\n🎯 Next Steps:")
        print("1. Homepage should now load without errors")
        print("2. Look for the Socket Debug Panel at the top (yellow box)")
        print("3. Watch the 'Total Connections' count - it should stay at 0 or 1")
        print("4. Use 'Connect Socket' button and watch connection count")
        print("5. Try creating a room and see what happens to connections")
        print("\n📋 What to Report:")
        print("  - Connection count number")
        print("  - Socket ID (should be consistent)")
        print("  - Any patterns in connection history")
        print("  - Console logs when clicking 'Log Socket Info'")
    else:
        print("\n❌ Replacement failed - check the file manually")
