#!/usr/bin/env python3
"""
Update Session Workflow Instructions - Windows Compatible
Adds directory mapping to session end procedures with proper encoding
"""
...
def update_development_plan():
    ...
    if not os.path.exists(dev_plan_path):
        print("WARNING: development_plan.md not found")
        return False
...
"""
    ...
    with open('SESSION_WORKFLOW.md', 'w', encoding='utf-8') as f:
...
def update_changelog():
    ...
    try:
        ...
        print(f"WARNING: Error updating changelog: {e}")
        return False
...
def main():
    ...
    if create_session_workflow_file():
        print("[SUCCESS] Created SESSION_WORKFLOW.md with comprehensive session procedures")
    
    # Update development plan
    if update_development_plan():
        print("[SUCCESS] Updated development_plan.md with session workflow reference")
    
    # Update README
    if update_readme():
        print("[SUCCESS] Updated README.md with session workflow reference")
    
    # Update changelog
    if update_changelog():
        print("[SUCCESS] Updated CHANGELOG.md with session workflow entry")
    
    print("\n[COMPLETE] Session Workflow Instructions Added!")
